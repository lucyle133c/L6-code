...
'ict-export_run-per-row.csv' file on 'http://10.20.199.186:8082/' to get the info
and add the location indo into the ICT Defect file. 
Servers: 8081-4 synced
...
# temp = "" --> Change path if needed

urllib.request.urlretrieve(url, os.path.join(
    temp_path, 'ict-export_run-per-row.csv'))


# # ***** If need to manually input *****
# Step 1: Change the week number (+ Step 2: Change the week number - AUTO)
week = 'WK15'
# Step 2: Change the week number
week1 = "'WK15'"
# Step 3: Change the start date
start_date = "'04/05/2021'"
# Step 4: Change the end date.The date should be the next day of the end date you want.
end_date = "'04/11/2021'"
...
BOT_AOI_Defect_Data.to_csv(os.path.join(
    save_path, f"BOT_AOI_{week}.csv"), index=False)
TOP_AOI_Defect_Data = TOP_AOI_Defect(start_date, end_date, week1)
TOP_AOI_Defect_Data.to_csv(os.path.join(
    save_path, f"TOP_AOI_{week}.csv"), index=False)
...
PACK_AOI_Defect_Data.to_csv(os.path.join(
    save_path, f"PACK_AOI_{week}.csv"), index=False)
...
defect_df.to_csv(
    f'./{week}/{week}/Customer/Defects/ICT_{week}.csv', index=False)
...
# week1 = "'" + week + "'"
# # Step 3: Change the start date
# o_start_date = datetime.date(2021, 4, 19)
