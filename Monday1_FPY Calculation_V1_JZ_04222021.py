...
# end_date = "'04/11/2021'"
# end_date1 = "'04/10/2021'"

week_date = pd.date_range(start_date, end_date1)

create_folder(week)  # Reminder: For each week, just run it for one time.
...
# The path of the station file
savepath = f'{path}/{week}/{week}/{station}/Fpy/Daily/'

for d in week_date:
    percent_df = calculate_fpy_daily(
        bot_aoi_pass_data, bot_aoi_fail_data, d, station)
...
for d in week_date:
    percent_df = calculate_fpy_daily(
        top_aoi_pass_data, top_aoi_fail_data, d, station)
    filename = savepath + str(d.strftime("%Y-%m-%d")) + '_fpy.csv'
    percent_df.to_csv(filename, index=False)
...
for d in week_date:
    percent_df = calculate_fpy_daily(
        PACK_AOI_pass_data, PACK_AOI_fail_data, d, station)
    filename = savepath + str(d.strftime("%Y-%m-%d")) + '_fpy.csv'
    percent_df.to_csv(filename, index=False)
...
for station in station_list:
    ...
    dailyfpy_path = os.path.join(
        path, f'{week}\\{week}\\{station}\\Fpy\\Daily\\')
    savepath1 = os.path.join(
        path, f'{week}\\{week}\\Customer\\Fpy\\{week}_{station}_fpy.csv')
...
# ******** Practice Code ********
# # Step 1: Change the week number
# week = 'WK15'
# # Step 2: Change the start date
# o_start_date = datetime.date(2021, 4, 12)
# start_date = "'" + o_start_date.strftime('%m/%d/%Y') + "'"
# # # Step 3: Change the end date - should be the next day of the end date you want.
# # If more than typical 1 week, change count to amount of days
# day_count = 5
# o_end_date = o_start_date + datetime.timedelta(days=day_count)
# end_date = "'" + o_end_date.strftime('%m/%d/%Y') + "'"
# # # Step 4: Change the end date.The date is the real end date you want.
# o_end_date = o_start_date + datetime.timedelta(days=day_count-1)
# end_date1 = "'" + o_end_date.strftime('%m/%d/%Y') + "'"
